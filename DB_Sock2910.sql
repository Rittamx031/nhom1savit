...
Create table Sock_Detail(
	...
	constraint fk_sockDeTail_size foreign key(id_size) references Size(id),
	constraint fk_sockDeTail_color foreign key(id_color) references Color(id)
)

Create table Discount_SockDetail(
	id uniqueidentifier primary key default NEWID(),
	id_sock_detail uniqueidentifier,
	id_discount uniqueidentifier,
	unit_base_price money not null,
	money_return money not null,
	[status] bit default 1,
	constraint fk_DisSockDT_SDT foreign key(id_sock_detail) references Sock_Detail(id),
	constraint fk_DisSockDT_Dis foreign key(id_discount) references Discount(id)
)
